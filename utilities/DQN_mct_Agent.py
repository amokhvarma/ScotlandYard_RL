...
import math
from utilities.mcts import mcts
    ...
    def replay(self):
        ...
        for (state_action, next_state, reward) in mini_batch:
            if (next_state.end_flag):
                ...
                #_, max_rew = self.mct_action(next_state, type)
                p = mcts(next_state,c=2,playouts=5)

                (best_action,t) = p.planning()
                if(len(best_action)==2):
                    new_best = (best_action[0],best_action[1],t[0])
                else:
                    new_best = (best_action[0],t[0])
                feature = next_state.f_x_action(new_best)
                feature_tensor = torch.from_numpy(feature)
                max_rew = self.model.forward(feature_tensor.float()).detach().numpy()[0][0]
